...
def getBehindTwoPoints(front_left_point: MyPoint, front_right_point: MyPoint, desk_plane: MyPlane) -> tuple:
    ...
    #n = np.array([A,B,C])
    #c = np.cross(a,n)
    #c = c/(np.linalg.norm(c))*dis
    
    #(x4,y4,z4) = (x1+c[0],y1+c[1],z1+c[2])
    #(x3,y3,z3) = (x2+c[0],y2+c[1],z2+c[2])
           
    
    results = solve([
        (x4 - x1) * (x1 - x2) + (y4 - y1) * (y1 - y2) + (z4 - z1) * (z1 - z2),
        ((x4 - x1) ** 2 + (y4 - y1) ** 2 + (z4 - z1) ** 2) ** 0.5 - a,
        A * x4 + B * y4 + C * z4 + D
    ], [x4, y4, z4])
    #print(results)
    ...
    #_x3 = front_right_point.x + behind_left_point.x- front_left_point.x
    #_y3 = front_right_point.y + behind_left_point.y- front_left_point.y
    #_z3 = front_right_point.z + behind_left_point.z- front_left_point.z
    behind_right_point = MyPoint(results[0]) if results[0][2] > results[1][2] else MyPoint(results[1])


    #behind_left_point = MyPoint([x4,y4,z4])
    #behind_right_point = MyPoint([x3,y3,z3])
    #print("two point is ")
    #print(behind_left_point,behind_right_point )
    return behind_left_point, behind_right_point 

def ransacLine(points, iter=3000, good_len=3, sample_points=8):
    '''
...
class Open3dCalc:
	...
	def planeSeg(self):#平面分割，在初始化之后调用，返回一个n*3的矩阵
		self.plane_model, self.inliers = self.pcd.segment_plane(distance_threshold=0.004	,
                                         ransac_n=700,
                                         num_iterations=1200)
	...
	def showStatic(self,_xyz):#可视化，传入一个n*3的矩阵,这个函数是阻塞的
		...
		self.outlier_cloud.paint_uniform_color([1, 0, 0])
...
def getDesk3d(dets,depth_frame,depth_intrin):#传入三维点集
	...
	a = Open3dCalc(dets)
	pic_raw = cv.imread('aa.jpg')
	for i in a.planeSeg():
	    temp=rs.rs2_project_point_to_pixel( depth_intrin,[i[0],i[1],i[2]] )
	    cv.circle(pic_raw,(round(temp[0]),round(temp[1]) ),1,(0,0,255))#画出平面点集

	kernel = cv.getStructuringElement(cv.MORPH_RECT, (2, 2))#定义结构元素的形状和大小
	pic_raw = cv.erode(pic_raw, kernel)#腐蚀操作
	pic_raw  = cv.blur(pic_raw,(2,2))#滤波
	...
	cv.circle(pic_raw,(left_x,left_y),2,(255,0,0))
	cv.circle(pic_raw,(right_x,right_y),2,(255,0,0))
	cv.circle(pic_raw,(bottom_x,bottom_y),2,(255,0,0))

	cv.circle(pic_line,(left_x,left_y),2,(100,1,100))
	cv.circle(pic_line,(right_x,right_y),2,(100,1,100))
	cv.circle(pic_line,(bottom_x,bottom_y),2,(100,1,100))	

	if 0.2>abs( abs(temp_left_vector[1][0]/temp_left_vector[0][0])-abs(temp_right_vector[1][0]/temp_right_vector[0][0]) ):
		cv.circle(pic_line,(right_x,right_y),5,(100,1,100))	
		cv.circle(pic_line,(left_x,left_y),5,(100,1,100))
		cv.circle(pic_raw,(right_x,right_y),5,(100,1,100))	
		cv.circle(pic_raw,(left_x,left_y),5,(100,1,100))
		#return left_x,left_y,right_x,right_y
	
	else:	
		if abs(temp_left_vector[1][0]/temp_left_vector[0][0]) < abs(temp_right_vector[1][0]/temp_right_vector[0][0]):
			cv.circle(pic_line,(bottom_x,bottom_y),5,(100,1,100))	
			cv.circle(pic_line,(left_x,left_y),5,(100,1,100))	
			cv.circle(pic_raw,(bottom_x,bottom_y),5,(100,1,100))	
			cv.circle(pic_raw,(left_x,left_y),5,(100,1,100))	
	...
	cv.circle(pic_raw,(int(left_behind_x),int(left_behind_y) ),5,(100,1,100))	
	cv.circle(pic_raw,(int(right_behind_x),int(right_behind_y) ),5,(100,1,100))

	#cv.imshow("pic_raw",pic_raw)
	#cv.imshow("pic_line",pic_line)
	#while True:
	#	cv.waitKey(1)
...
# print( getDesk3d(dets,depth_frame,depth_intrin) ) #传回四个list变量：左前三维点，右前三维点，左后三维点，右后三维点
